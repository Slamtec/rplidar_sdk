...
LRESULT CIpConfigDlg::OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    ...
    u_result ans = LidarMgr::GetInstance().lidar_drv->setLidarIpConf(manual_conf);
    if (IS_FAIL(ans)) {
...
LRESULT CIpConfigDlg::OnDhcpClient(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    ...
    u_result ans = LidarMgr::GetInstance().lidar_drv->setLidarIpConf(client_conf);
    if (IS_FAIL(ans)) {
